...
function attachAPI<T extends APISchema>(
  ...
  for (const apiName in apis) {
    ...
    // @ts-ignore
    hostApi[apiName] = (params, options): Promise<T> => {
...
// 默认客户端配置
export function request<T extends APISchema>(config:CreateRequestConfig<T>):CreateRequestClient<T>{
...
export function download<T extends APISchema>(config:CreateRequestConfig<T>):CreateRequestClient<T>{
  return createRequestClient<T>({
    baseURL: import.meta.env.RENDERER_VITE_BASE_URL?'':import.meta.env.RENDERER_VITE_BASE_URL,
    headers: {
      'Content-Type': 'application/json'
